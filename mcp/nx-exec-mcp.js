// nx-exec-mcp.js — NX-PLUGIN execution layer MCP adapter (MCP v5)
...
// Usage: register via .mcp.json stdio: node nx-exec-mcp.js
...
function launchNx(nxPath, partPath, customDir) {
  return new Promise((resolve, reject) => {
    ...
    console.error("[nx-exec] nx_open: launching " + exe + " " + args.join(" "));
    const child = exec(`"${exe}" ${args.join(" ")}`, opts, (err) => {
      if (err && err.code !== 0 && err.code !== null) {
        console.error("[nx-exec] nx_open: launch error:", err.message);
      }
    });
    child.unref();
...
const INSTRUCTIONS = [
  "nx-exec = NX-PLUGIN execution layer (TCP 1977 -> C# DLL -> NXOpen API), NX operation tools.",
...
function handle(req) {
  ...
  if (method === "server/discover") {
    ...
    send(id, { result: { protocolVersion: negotiateVersion(clientVer), capabilities: { tools: { unordered: true } }, serverInfo: { name: "nx-exec", version: "1.0.1" } } });
  } else if (method === "initialize") {
    const clientVer = params?._meta?.io?.modelcontextprotocol?.protocolVersion || params?.protocolVersion;
    send(id, { result: { protocolVersion: negotiateVersion(clientVer), capabilities: { tools: { unordered: true } }, serverInfo: { name: "nx-exec", version: "1.0.1" }, instructions: INSTRUCTIONS } });
    ...
    pendingTools.then(tools => {
      const list = tools.map(t => {
        ...
        return {
          ...
          annotations: { readOnlyHint: false }  // nx-exec tools modify NX model state
        };
      });
...
const nxExecTransport = startCompatibleStdio({
  onmessage: (msg) => {
    try { handle(msg); } catch (e) { console.error("[nx-exec] handle error:", e.message); }
  },
  onerror: (err) => {
    console.error("[nx-exec] stdio error:", err.message);
  },
  onclose: () => {
    console.error("[nx-exec] stdin closed - exiting.");
    process.exit(0);
  },
});
